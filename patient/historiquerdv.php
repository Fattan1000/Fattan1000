            ...
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                <tr >
                    ...
                    <td width="15%">
                       ...
                       $list110 =mysqli_query($conn,"select * from rendez_vouz where patient_id=$patid");
               ...
               $sqlmain = "SELECT rendez_vouz.id,rendez_vouz.description,date_rdv,rendez_vouz.status,medecin.nom,heure_debut_rdv, medecin.prenom, medecin.email FROM medecin INNER JOIN rendez_vouz ON rendez_vouz.medecin_id = medecin.id WHERE  rendez_vouz.patient_id = $patid  ORDER BY date_rdv desC";
                            ...
                            <?php
                                ...
                                for ( $x=0; $x<$result->num_rows;$x++){
                                    ...
                                    echo '<tr >
                                       ...
                                       &nbsp;&nbsp;&nbsp;
                                       <a href="?action=drop&id='.$idRDV.'&name='.$medecinnom.'" class="non-style-link"><button  class="btn-primary-soft btn button-icon btn-delete"  style="padding-left: 40px;padding-top: 12px;padding-bottom: 12px;margin-top: 10px;"><font class="tn-in-text">supprimer</font></button></a>
                                       &nbsp;&nbsp;&nbsp;</div>
                                        </td>
                                    </tr>';
    ...
    $id=$_GET["id"];
      ...
      if($action=='drop'){
                                        ...
                                        '; if(isset($_POST["annuler"])){
                                            mysqli_query($conn,"DELETE from rendez_vouz
                                            WHERE id=$id;");  echo"<script>alert('ce rendez vous a été supprimer')</script>"; }
                                
                                    }
elseif($action=='view'){//$sqlmain= "select * from medecin where id='$id'";
    $sqlmain = "SELECT rendez_vouz.id,date_rdv, medecin.nom, medecin.email, rendez_vouz.description,rendez_vouz.status,rendez_vouz.heure_debut_rdv, medecin.prenom, medecin.specialite FROM medecin INNER JOIN rendez_vouz ON rendez_vouz.medecin_id = medecin.id WHERE rendez_vouz.id = $id";
  ...
  $medecinprenom=$row["prenom"];
    ...
    <center>
      ...
      <a href="fichemedicale.php?id='.$rdvid.'" class="non-style-link">
